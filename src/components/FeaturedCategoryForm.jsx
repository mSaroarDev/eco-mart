...
export default function FeaturedCategoryPopup({ action }) {
  // toast
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      if (
        ...
        setLoading(true);
        const res = await fetch("/api/category/featured/new", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
        ...
        setLoading(false);
        if (res.status === 406) {
          showError("Duplicate Reques, Change order now");
        } else if (res.status === 201) {
          showSuccess("Category Featured.");
          router.refresh();
          action();
        } else {
    ...
    <>
      ...
      <div className="popup-overlay">
        <div className="h-screen w-full flex items-center justify-center">
          <div className="popup-box w-[700px]">
            {loading && <Spinner />}
                ...
                <div className="w-full mt-5">
                  ...
                  >
                    ...
                    <div className="col-span-9 mt-4">
                      <div className="w-full flex flex-col login">
                        <label htmlFor="name">Category Link</label>
                        <input
                          type="text"
                          id="category_link"
                          name="category_link"
                          value={formik.values.category_link}
                          onChange={formik.handleChange}
                          className="login-input"
                        />
                      </div>
